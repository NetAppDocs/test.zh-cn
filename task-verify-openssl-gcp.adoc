...
summary: 使用OpenSSL根据摘要文件内容验证Google Cloud下载的disk.raw文件。 
---
= 使用OpenSSL验证disk.raw文件和摘要文件内容
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以根据通过提供的摘要文件内容验证Google Cloud下载的disk.raw文件 https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab["NSS"^] 使用OpenSSL。


NOTE: 用于验证映像的OpenSSL命令与Linux、Mac OS和Windows计算机兼容。
...
----
====
. 将下载的disk.raw文件、签名和证书置于目录中。
. 使用OpenSSL从证书中提取公共密钥。
. 使用提取的公共密钥对签名进行解密、并验证下载的disk.raw文件的内容。
+
.单击以显示脚本
[%collapsible]
====
